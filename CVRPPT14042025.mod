...
#subject to use_one_direction {(i,j) in A2}:
#	y[i,j] + y[j,i] <= 1;
...
subject to C15 {k in K}:
	sum{i in N1,j in C_H union C_F: (i,j) in A1}(d[j]*x[i,j,k])+sum{i in N1,j in L: (i,j)in A1}(sum{c in C_S union C_F}d[c]*u[c,j]*x[i,j,k])
