...
#set M_t;													#Set of starting MRT stations
#set M_e;													#Set of end MRT stations
...
#param visit_M_penalty default 100;  # Penalize if not visited
...
#param r{M,M};												#i station is related to j end station, then 1 otherwise 0   
...
#var c1{M}>=0;												#MRT depart from node i
#var slack13{i in N1, j in N2: (i,j) in A1} >= 0;
...
#+sum {m in M} visit_M_penalty * (1 - g[m]);  # Encourages visiting MRT stations
...
#subject to MRT_direction_exclusive {(i,j) in A2: (j,i) in A2 and i < j}:
#  y[i,j] + y[j,i] <= 1;
...
#subject to C13_fix {i in N1, j in N2: (i,j) in A1}:
#  b[j] >= b[i] + t[i,j] + s[j] - BigM*(1 - sum{k in K} x[i,j,k]) - slack13[i,j];
...
#subject to C25 {i in M, j in M : (i,j) in A2}:
#    	y[i,j] <= r[i,j];
    	
#subject to MRT_from_vehicle_only {(i,j) in A2}:
#  y[i,j] <= g[i];
  #y[i,j] <= g[i] + g[j];
  #y[i,j] <= max(g[i], g[j])
